...
import {
  MapPin, Search, Store, Star, Clock, Bike, SearchX,
  UtensilsCrossed, Sandwich, Pizza, Drumstick, Cake, Fish,
} from 'lucide-react'
...
const CATEGORIES = [
  ...
  { id: 'doner', name: 'Döner', Icon: Sandwich },
  { id: 'pizza', name: 'Pizza', Icon: Pizza },
  { id: 'kebap', name: 'Kebap', Icon: Drumstick },
  { id: 'tatli', name: 'Tatlı', Icon: Cake },
  { id: 'deniz', name: 'Deniz Ürünleri', Icon: Fish },
]
        ...
        <div className="max-w-4xl mx-auto p-4 space-y-3">
          ...
          <div className="relative group">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3.5 bg-slate-100 border-2 border-transparent rounded-md text-slate-900 placeholder-slate-500 focus:bg-white focus:border-orange-500/30 focus:ring-4 focus:ring-orange-500/10 transition-all outline-none"
            />
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto p-4 pt-6 space-y-8">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between px-1">
            ...
            <button className="text-orange-600 text-xs font-bold hover:bg-orange-50 px-3 py-1.5 rounded-md transition-colors">
              Tümünü Gör
            </button>
          </div>
          <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0">
            {CATEGORIES.map((cat) => (
              <button
                ...
                className={`flex flex-col items-center justify-center min-w-[90px] aspect-square rounded-md transition-all duration-300 shadow-sm ${
                  selectedCategory === cat.id
                    ? 'bg-orange-500 text-white shadow-sm shadow-orange-500/40 -translate-y-2'
                    : 'bg-white text-slate-600 hover:bg-orange-50 shadow-sm shadow-slate-200/50 border border-slate-100 hover:-translate-y-1'
                }`}
              >
                <cat.Icon size={28} strokeWidth={1.5} className="mb-2" />
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between px-1">
            ...
            <div className="bg-white border border-slate-100 p-1.5 rounded-md shadow-sm flex items-center gap-1">
              <button className="bg-slate-50 text-slate-400 p-2 rounded-md">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 4.5h18M3 12h18M3 19.5h18" />
                </svg>
              </button>
              <button className="bg-orange-500 text-white p-2 rounded-md shadow-sm">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {[1, 2, 3, 4].map(i => (
                <div key={i} className="h-[280px] bg-white border border-slate-100 rounded-md p-4 flex flex-col gap-3">
                  <div className="w-full h-40 bg-slate-100 animate-pulse rounded-md" />
              ...
              <AnimatePresence mode="popLayout">
                {filteredRestaurants.map((res) => (
                  <motion.div
                    ...
                    key={res.id}
                    className="bg-white rounded-md overflow-hidden shadow-sm shadow-slate-200/50 border border-slate-100 group hover:shadow-sm hover:shadow-orange-500/10 transition-all duration-500"
                  >
                    <div className="h-44 bg-slate-100 relative overflow-hidden">
                      {res.logo_url ? (
                        <img src={res.logo_url} alt={res.name} className="w-full h-full object-cover transition-transform duration-700" />
                      ) : (
                      ...
                      <div className="absolute top-4 right-4 flex flex-col gap-2">
                        <div className="bg-white/95 backdrop-blur-md px-3 py-1.5 rounded-md text-xs font-black text-slate-900 flex items-center gap-1 shadow-sm">
                          <Star size={12} strokeWidth={1.5} className="text-orange-500 fill-orange-500" /> {res.rating}
                        </div>
                        <div className="bg-orange-500/95 backdrop-blur-md px-3 py-1.5 rounded-md text-[10px] font-black text-white shadow-sm uppercase tracking-wider">
                          %20 İndirim
                        </div>
                      </div>
                    </div>
                    <div className="p-6">
                      ...
                      <div className="flex items-center gap-4 text-[11px] text-slate-500 font-bold uppercase tracking-wide">
                        <span className="flex items-center gap-1.5 bg-slate-50 px-2.5 py-1 rounded-md">
                          <Clock size={14} strokeWidth={1.5} /> {res.delivery_time}
                        </span>
                        <span className="flex items-center gap-1.5 bg-slate-50 px-2.5 py-1 rounded-md">
                          <Bike size={14} strokeWidth={1.5} /> {res.delivery_fee === 0 ? 'Ücretsiz' : `${res.delivery_fee} ₺`}
                        </span>
                      </div>
                      <div className="mt-5 flex items-center justify-between pt-4 border-t border-slate-50">
                        ...
                        <Link 
                          ...
                          className="bg-slate-900 text-white px-5 py-2.5 rounded-md text-xs font-bold hover:bg-orange-600 shadow-sm shadow-slate-900/10 hover:shadow-orange-500/30 transition-all"
                        >
                          Sipariş Ver
                        </Link>
                      </div>
          ...
          {!isLoading && filteredRestaurants.length === 0 && (
            <div className="py-20 text-center space-y-3">
              <div className="flex justify-center">
                <SearchX size={32} strokeWidth={1.5} className="text-slate-400" />
              </div>
